...
class AbnormalReportActivity : VbBaseActivity<AbnormalReportViewModel, ActivityAbnormalReportBinding>(), OnClickListener {
    ...
    override fun initListener() {
        ...
        ClickUtils.applySingleDebouncing(binding.rflRecognize, 1000, this)
        ClickUtils.applySingleDebouncing(binding.rflTakePhoto, 1000, this)
        ClickUtils.applySingleDebouncing(binding.rflTakePhoto2, 1000, this)
        binding.ivPlateDelete.setOnClickListener(this)
        binding.ivPanoramaDelete.setOnClickListener(this)
        ClickUtils.applySingleDebouncing(binding.rivPlate, 1000, this)
        ClickUtils.applySingleDebouncing(binding.rivPanorama, 1000, this)
        ...
        if (resultCode == RESULT_OK) {
            if (requestCode == 1) {
                ...
                if (!plate.isNullOrEmpty()) {
                    binding.etPlate.setText(plate)
                    binding.etPlate.setSelection(plate.length)
                    when (plateColor) {
